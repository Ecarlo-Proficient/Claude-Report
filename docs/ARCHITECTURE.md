...
Last updated: 2026-09-25 (ledger/: NEW `reapply_check.py` - **Re-apply in QuickBooks** on Checks QBO changed: a stripped check goes back on the bills it paid (the change log's before copy, or a saved ledger copy for pre-log strips), dry run -> "Are you sure?" -> write, the ledger's one QBO write; the page's chips cut to one row (To fix · Unapplied · Moved to a newer bill · Credit · Resolved · History + vendor type). Same day, ledger/: NEW **strip history** - `ledger/strip_history.py`: every paid check QBO left with no bills, kept for good after the fix (the mirror's change log + deleted paid bills + pre-log strips from `CompanyHealth/check_strip_case.json`), trigger / status / re-applied date / "paid again" double payments, time windows; the Checks QBO changed page gains a **History** chip (`/api/checkstrips`) and a **QBO report (PDF)** button that rebuilds the support report from it (`/api/checkstrips/pdf`, Chrome-printed to CompanyHealth). Previously 2026-09-24 (ledger/: NEW **Uncleared checks** page - `ledger/load_uncleared_checks.py` pulls QBO's TransactionList `cleared=Uncleared` (the filter works though the column is hidden) + each bank account's matched-through date -> `uncleared_check` / `bank_match`, `/api/uncleared`; QBO changes page folds the bills a check lost under that check (`_fold_side_effects`) and sends the fix to Checks QBO changed; ledger/: NEW **Checks QBO changed** audit page (Company, beside QBO changes) - `ledger/check_drift.py` over the mirror, every year on file: checks QBO rewrote after they were paid (floating unapplied money; the paid bill edited and open again; its deleted-and-re-entered copy open; the freed vendor credit; floating money put on a bill dated after the check), one open bill per check (exact amount first), subs first, the fix per check; `/api/checkdrift`; `python3 ledger/check_drift.py --check <#>` prints one check's story; python-env/: every tool on ONE pinned Python + the interpreter guard; ledger/sync_view.py: sync-all's mirror + ledger-reload steps as one progress line per step, the AP/AR look, raw output to ~/Library/Logs/Proficient/ledger-sync/; preflight gate 5/5 = the tests). Previously 2026-09-23 (shared/paths: `MOUNTS` + `require_mounts` - sync-all / sync-ap / sync-ar / the ledger reload STOP before any work when the Accounting share or OneDrive is missing; shared/qbo_mirror: NEW **change log** `mirror_change` - every refresh diffs each record against the copy held: created / edited / deleted / restored, QBO's own time, plain-word flags (`change_flags`), the encrypted BEFORE record (`change_before`), `backfill_changes`, `refresh_mirror.py --changes`; ledger/: **four views** - Projects · Vendors (Bill Tracker · Vendor Center) · Customers (Invoice Tracker · Customer Center · Payments received · Sales pipeline) · Company (Money · **QBO Audit** = `/api/qboaudit` over the change log + the Bill Tracker audits, `mirror` pipeline = Refresh from QuickBooks); project page: no equation under Coverage, All draws lights the table, a click never scrolls; P&L invoices fold with memo + paid date; compact one-line stats app-wide; LEDGER_VERSION 1.5.0). Previously 2026-09-22 (ledger/: NEW **bill payment stub** - `ledger/bill_payment_stub.py`, one stub per QBO BillPayment (payment block on top, the bills it paid below, QBO's report columns as a registry with add/drop), mirror-only, Chrome-printed PDF into Accounting/Accounts Payable/Bill Payment Stubs/<vendor>/; wired into the vendor page's Payments view (Print stub · Stub columns picker · print history with an archived copy per print and a live status vs QBO: current / changed / voided / deleted; `bill_payment_stub` table; `/api/bill-payment/stub|stubs|stub/file`; LEDGER_VERSION 1.4.0). Previously 2026-09-18 (bill-tracker/: NEW **`Bill List`** sheet (one row per bill, `Approved` = approved / not approved / check QBO - QBO's API does not return bill approval status) + **Revised** issue in `Audit - Bills` (`bill_rows.approval_state` / `revised_reason`); `shared/xlsx_verify` unescapes XML before the header-vs-table check. Same day, ledger/: **Systems rows link to their one-page process guide** - the vault gained `assets/processes/` (`<PROCESS-ID>_<slug>.pdf` + `.html`, built with the `pdf-handout` skill); `shared/paths.process_guides_dir()`, `registry_view.guides()` / `guide_path()`, `/api/process-guide?id=&fmt=`, looked up by ID, read-only, no cache. Previously 2026-09-16 (ledger/: the **RP review became the WIP review, one page per division** - `/api/review?div=RP|CP|MFD` + `/api/review/mark|refresh`, Me / PM modes, `rp_review.division_cards()` adapts the WIP tools' `--emit-review` JSON for CP / MFD (contract + COs from the draw G702, ETC from the takeoff, QBO costs / billed); the **project page's funding section** groups invoices into draws by the named draw month or the invoice date (`_project_invoices` / `_draws_by_invoice` / `_recount_draw`), reads as the equation, keeps Coverage on top, and gates the pay run behind "Pay bills" with a Save / Discard draft; the vendor page carries project + client per bill and the bills each BillPayment paid. Previously 2026-09-15 (ledger/: NEW **RP review page** - `ledger/rp_review.py` builds `rp-review/rp_review.json` (every RP line's numbers + source + a 5-row cut of the source sheet, JobTread via `shared/jobtread.py`, the Removed log), `/api/rp/review|answers|mark|refresh`; answers in `rp_review_mark` + `_log` with the Me / OPS Manager stamp. Previously 2026-09-02 (ledger/: NEW **money trail** - `ledger/trail.py` + `static/trail.js`, `/api/trail`: every QBO line behind a project's Costs / Billed with the running total against ETC and contract; `cost_line` gained bill #, memo, line #, bill total, vendor/class ids, sub evidence, scan flag. 2026-08-25: NEW **Graph tab** — the org as a map (`ledger/vault_graph.py`,
...
├─ rp_price.py         the ONE RP contract/ETC resolver: NEWEST DOCUMENT WINS (JobTread vs proposal PDF vs takeoff), guards; wip/rp_price_sync writes the RP file (2026-09-17)
...
flowchart LR
    ...
    BT["excel_bill_sync.py\nBills/Inventory/Liens + Bill List (simple, 3-state Approved)\n+ 3 themed Audit sheets (Bills: Not Approved · Revised · Duplicate) + History log"]:::tool
...
flowchart LR
    ...
    DASH["dashboard.py + static/\nlocal web UI (127.0.0.1) - READ-ONLY except the owner's marks + WIP Review writes.\nFOUR views (2026-09-23): Projects = the WIP as the page (row → project page)\n· Vendors = Bill Tracker / Vendor Center · Customers = Invoice Tracker / Customer Center / Payments / Sales\n· Company = Money / QBO Audit (the mirror change log) · the gear = sync + WIP Review + Console + Systems"]:::tool
    ...
    MIRROR[("qbo_mirror.sqlite3\nTHE raw mirror: 21 entities, full JSON,\ndeletes flagged · seed once, /cdc refresh in seconds\n+ mirror_change: every refresh diffs each record - created / edited / deleted / restored,\nQBO time, plain-word flags, the encrypted BEFORE record (2026-09-23)")]:::src
    MREF["refresh_mirror.py\n--seed · refresh (change feed, logs changes) · --reconcile · --status · --changes N"]:::tool
    QBO --> MREF --> MIRROR
    MIRROR -- "every query_all in the repo is answered here\n(shared/qbo_api routes; ACB_QBO_LIVE=1 = live)" --> COSTLOAD
    SYNCALL["ledger/sync_all.sh = the sync-all alias\nmount preflight (shared/paths --require: Accounting share + OneDrive) = STOP before any work\n0 mirror refresh · 1 AP · 2 AR · 3 ledger reload\nsteps 0 + 3 shown by ledger/sync_view.py: one line per step, like AP/AR"]:::tool
    SYNCALL --> MREF
    ...
    STUB["bill_payment_stub.py\nthe check / bill-payment stub: payment on top, bills below (QBO look)\ncolumn registry · mirror only · Chrome -> PDF in Accounting/Accounts Payable/Bill Payment Stubs/<vendor>/\n+ bill_payment_stub HISTORY (snapshot + archived PDF per print, status vs QBO now)\nvendor page: payments GROUPED (bills as the expansion) · checkboxes -> Print N stubs · VOIDED pill\nbill tables flat like the Excel + Excel-style header filters (hfDecorate) + search\nPrint stub · Stub columns · /api/bill-payment/stub|stubs|stub/file"]:::tool
    ...
    MIRROR -->|"BillPayment + Bill + Vendor"| STUB
    STUB -->|"Print stub / history"| DASH
    QBO --> ATT --> DB
    NOTIONP[("Notion Invoice Tracker\n(page read, on demand)")]:::src --> NPAGE --> DASH
    MIRROR -.->|"/api/qboaudit - the QBO Audit page: what QBO deleted / edited / unapplied (read-only)"| DASH
    DRIFT["check_drift.py\nChecks QBO changed: paid checks QBO rewrote -\nfloating · bill open again (edited / re-entered) · credit freed · put on a later bill"]:::tool
    MIRROR -->|"BillPayment + Bill + VendorCredit + change log (read-only)"| DRIFT
    DRIFT -.->|"/api/checkdrift - the Checks QBO changed audit page"| DASH
    REAPPLY["reapply_check.py\nRe-apply in QuickBooks: a stripped check back on its bills\n(change log before copy / saved ledger copy)"]:::tool
    MIRROR -->|"change log before copy"| REAPPLY
    DASH -->|"/api/checkdrift/reapply: dry run, then confirmed write"| REAPPLY
    REAPPLY ==>|"BillPayment Line - owner confirmed"| QBO
    STRIPS["strip_history.py\nevery check left with no bills, kept after the fix -\ntrigger · status · paid again · the QBO support PDF"]:::tool
    MIRROR -->|"change log + BillPayment + Bill (read-only)"| STRIPS
    CASEF[("CompanyHealth/check_strip_case.json\npre-log strips + report narrative")]:::src --> STRIPS
    STRIPS -.->|"/api/checkstrips (History) · /api/checkstrips/pdf (QBO report)"| DASH
    UNCLR["load_uncleared_checks.py\nQBO TransactionList cleared=Uncleared\n+ matched-through per bank account"]:::tool
    QBO -->|"reports (read-only, Touch ID)"| UNCLR
    UNCLR -->|"uncleared_check + bank_match"| DASH
...
paid to the director; every line carries a CATEGORY and who it goes to (`bizdev_cut.categorize`, rules in the register) and each job sheet closes with the split into his / the junior estimator's / to confirm and a by-category block; the page keeps one combined column. No overhead model here - that is the owner's separate `<DIV> OH
