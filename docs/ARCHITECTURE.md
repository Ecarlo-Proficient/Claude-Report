...
flowchart LR
    ...
    DRIFT["one-offs/bill_payment_drift_audit.py\nchecks QBO rewrote after payment:\nfloating · auto-applied to a later bill · copy open · credit freed"]:::tool
    MIRROR -->|"BillPayment + Bill + VendorCredit + change log (read-only)"| DRIFT
    DRIFT --> DRIFTX[("CompanyHealth/\nBill Payment Drift.xlsx")]:::out
