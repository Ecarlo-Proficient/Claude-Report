# AGENTS.md — Automate Concrete Business (repo operating manual)
...
Company identity, divisions, and finance vocabulary live in the **global AGENTS.md** — don't
...
4. **Logs and data dumps go to `~/Library/Logs/Proficient/`** — never inside this folder (it's Codex-visible/synced).
...
- **Bills carry the project # in memo / PrivateNote** (not a period tag); subs are flagged by `"sub"` in
  the bill memo.
- **Codex's QBO connector P&L (for analysis) caps at 100 rows and doubles monthly totals, and name-keyed
...
- **Obsidian Main Vault is read-only** (`~/Library/Mobile Documents/com~apple~CloudDocs/Documents/Main Vault`
  — note the global AGENTS.md path is stale). Draft vault content into `AI Brain_Vault/drafts/vault-fills/`
...
Find most efficient way to automate tasks for concrete construction contractor business. 
Programs Used: QuickBooks Enterprise Suite
Notion, Teams, Excels Shared
