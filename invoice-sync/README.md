...
- **`Open_Invoices.xlsx` has four tabs.** `Open Invoices` is the flat mirror. `CP Aging`, `MFD Aging` and `RP Aging` are the owner's collections view, **one tab per division — there is no Division column, the tab is the division**: Current / 1-30 / 31-60 / 61-90 / 90+ buckets aged **by due date**, invoices grouped under the parent client and **collapsed by default**, a **`Lien`** column with the Ch. 53 notice deadline, the collections clerk's `Quick Status` as Notes, `Invoice #` hyperlinked to QBO, and **litigation invoices excluded**. RP drops the previous-draw columns (RP doesn't bill in draws).
- **The `Lien` column is a deadline watchlist, not legal advice.** Dates come from `shared/lien_clock.py` (shared with `health-dashboard/money_bleeds.py` so the two can't drift): CP/MFD notice by the 15th of the **3rd** month after the work month, RP by the 15th of the **2nd**, rolled **backward** off weekends. **Work month = invoice month** (the user 2026-07-16, settled). Retainage is flagged and given no monthly date — it runs its own track (§ 53.057). Confirm project type, parcel and owning entity before anything is actually sent.
