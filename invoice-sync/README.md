...
- **`Open_Invoices.xlsx` has three tabs.** `Open Invoices` is the flat mirror. `AR Aging` (added 2026-08-05) is the owner's collections view: Current / 1-30 / 31-60 / 61-90 / 90+ buckets aged **by due date**, RP + CP + MFD together with a `Division` filter, invoices grouped under the parent client and **collapsed by default**, the collections clerk's `Quick Status` as Notes, `Invoice #` hyperlinked to QBO, and **litigation invoices excluded**. `RP Aging` is the same view filtered to RP with the previous-draw columns dropped (RP doesn't bill in draws).
