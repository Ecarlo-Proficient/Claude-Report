...
- 2026-09-25 · **Vendor Payments: Refresh from QuickBooks** (owner: "refresh the vendor payments so i can get the ones we
  ...
  vendor. ~15 s; the button/bar are built once so a filter re-render mid-run keeps the progress. Not in reload/all.
...
- 2026-09-24 · **Checks QBO changed: what happened, per check - facts only** (owner: "only qbo" / "you are doing too
  ...
  never changed - the check lost it) · put on a later bill · credit dropped · before the change log (no evidence, never
  guessed - a bill stamped minutes before its check is not proof of an edit). A strip the change log caught rebuilds
  the exact bills from the check's before copy. Times shown in Central (QBO stamps Pacific). **No cross-check patterns
  (bursts, bill profiles):** the list is only what is still unfixed, so any trend across it is survivorship, not
  signal. The change log keeps every strip even after it is fixed - a trend read belongs there, later, if asked.
...
- **sync-all steps 0 and 3 show progress, not data (2026-09-24, owner: "3 & 4 is giving too much info ... i need it like the 1/2").** NEW `ledger/sync_view.py`: the mirror refresh (`mirror`, + the Sunday reconcile, never after a failed refresh) and the ledger reload (`reload`) print one line per step with a count and a check-mark, framed like sync-ap / sync-ar. A failed step shows ✗ + the reason and writes a crash report; the full raw output goes to `~/Library/Logs/Proficient/ledger-sync/run.log`. `reload_ledger.sh` wraps each loader in `run` (`-- label --` / `[step-exit N]`, what the view reads), hands a terminal run to the view, stays plain for pipes or `ACB_PLAIN=1`, and skips its drive check when sync-all already did it (`ACB_DRIVES_CHECKED`), so no prompt can hide behind the view. sync-all's trailing mirror at-rest line is gone. **Fix:** `load_wip_master.py` now honors `ACB_LEDGER_DB` like every other loader (a test reload against a scratch DB still wrote this one file to the real ledger).
...
- 2026-09-08 · Overview is a BOARD: 12-column card grid, each widget S/M/L/Full and show/hide from its header tools or Settings > Overview widgets (`WIDGET_DEFS`, `settings.sizes`), hidden widgets return from a "+ name" strip; mono `//` headers and mono big numbers, theme backgrounds untouched. Invoice-page vendor bands: paid x/y (check mark when all paid) · still owed · total. WCAG AA audit, both themes, every tab: new tokens `--warn-text`, `--accent-text`, `--neg-text` (amber / accent / red as TEXT, per theme), light `--text-dim` and `--neg` deepened for tinted cells, dark `--neg` lifted, no opacity-ghosted text, nothing under 12px, qb badge green darkened; scanner is a JS tree walk with effective-background compositing (run it with the pane rendered - hidden panes freeze transitioned backgrounds).

- 2026-09-08 · project page: draws are TABS - a Coverage table (one row per draw: net billed, GC, costs, gross, margin, overhead, net, stage + All draws total) is the table of contents, each draw opens alone, left/right arrows flip (`_ppTabs`, `_ppCoverage`, `_pp.view`); the Coverage|Vendors|Bills toggle acts on the open draw. P&L reads billed (gross, WIP) - retainage held (WIP) = net billed (QBO invoices) - costs - overhead = net, on the project page block, its KPI strip, `_project_pnl` and `_portfolio_pnl` (the Financials tab tiles/columns say Net billed; `earned` stays in the payload, unused for net). P&L columns: a caption never strands away from its list.

- 2026-09-08 · ONE grouping rule (owner: "every single row that has a parent must be grouped"): `GRP_KINDS` in app.js + a MutationObserver decorate every group header the renderers emit (caret, remembered open/closed per header NAME in localStorage, children hidden while closed); headers with their own onclick are left alone. Kinds: table bands (`tr.bill-group`, `tr.sys-group`, trail `tr.tr-msum`), P&L `.pnl-codegrp` (cost codes collapse to job type by default), WIP Review divisions, record-panel `.dgroup > h4`, Sales `.warm-head`, Console `.pl-head`. The trail emits the MONTH header first (lines, total, running line at month end). Global `[hidden] { display: none !important }` - rows with their own display rule ignored the attribute. Register any NEW grouped list in `GRP_KINDS`. P&L: net billed = max(QBO invoices, WIP gross - retainage) with the source on the page (`billed_src`) - `billing_event` holds only the loaded invoice window, MFD177 read a 6M loss on 2.2M of 8.6M billed; by_code splits sub / non-sub per code; "(uncoded)" is labelled account-coded.

- 2026-09-08 · P&L parity (owner: "the ledger should be the P&L with a faster UI"): `load_invoices.load_history_from_qbo` lands EVERY QuickBooks invoice on a known project (source `qbo_history`, tracker rows win, `--no-history` to skip) - 18,999 older invoices on first run; net billed = the QuickBooks invoices only, with a `billed_gap` WARNING when the WIP report shows more (no silent WIP substitute). Uncoded cost lines group by the ACCOUNT they were entered under ("No cost code" > Concrete / Labor / Rebar...). Project page carries the job's Bill Tracker audit findings grouped by issue (`audits` in `/api/project/page`; `_fetch_accounting_audits` cached per workbook mtime).

- 2026-09-08 · Step 0: em-dash sweep of the whole ledger (UI strings, labels, comments, the empty-value glyph is now an en dash; stage "Fund in - pay vendors"); the Bill Tracker workbook still writes "33404 — RP…" so `load_bill_tracker` normalises it to " - " on the way in and the three parsers split on any dash (escaped `\u2014\u2013-` classes - a global replace once bent a literal class into an invalid range). Invoices tab: neutral client bands, the OPEN client in the accent (`.on`), picked rows keep an accent edge, each project inside a client is a 2px box (`in-proj` / `proj-last`), an RP project with one invoice lists the invoice without a band, open amounts sit in a black line (`amt-box`, band `boxed`). Financials by-job peek: three columns in a 460px scrolling box.

- 2026-09-08 · R1 trust-labels: source + as-of chip on every KPI strip and note (Margins & burn, Needs attention, Customer Center incl. the cash-in tiles as PROJECTIONS with a click-through list, Funding tiles + note, Sales/Vendors/Bills/Audit notes, P&L tiles, project page funding header); By division says its costs are the WIP report's; P&L division rows filter the jobs; `syncedAt(src)` reads the source file stamps; the project page KPI explains WHY QuickBooks and the WIP differ (`since_wip`: lines / invoices dated after the report date vs coding / scope).

- 2026-09-08 · R2 wip-review-truth: `/api/wip/review` stamps every division with its own compute time, marks one more than a day older than the newest STALE with the reason (CP: the Common drive not mounted, or "not recomputed since"), and reports whether the CP source is mounted; the tab shows the per-division as-of (red when stale) in the header and on each division band, and Sync warns before writing a stale division. Health and the WIP master already ride the Resync/reload chain. CP re-emitted 09/08 once Common was mounted.

- 2026-09-08 · R3 draw-table-and-visuals: draw bands say "N bills moved in from the draw before / moved to the next draw" (tooltip + face line in plain words); the pay-run column reads "none of N ticked / x of N ticked / nothing to pay"; funding header counts match the P&L block ("4 draws invoiced + 1 not yet drawn"); the next draw's stage reads "Blocked - pay $X on earlier draws first" when the funding chain blocks it. Visuals: cost-mix palette no longer reuses the accent / pos / neg hexes; client-statement copy uses money(); the scan menu uses _placeMenu; WIP and pay-run grids follow the density setting (`--row-pad-dense`) at ONE dense grid size (.88em); the draw-strip KPI override, duplicate .btn.primary and .btn.tiny are gone; vendor page shows the skeleton while loading; seven dead functions removed; stale date comment fixed.

- 2026-09-08 · Audit tab shows what QuickBooks HAS: the Class column is the QBO class parsed off the audit Detail (`qbo_class`), red with a tooltip when it is not the job's division; "Copy for clerk" puts one plain sentence per selected finding on the clipboard (bill, vendor, date, memo, amount, code -> what to change), e.g. "Class is Commercial in QuickBooks - change it to Residential (RP7608 is a Residential job)".

- 2026-09-17 · Health tab: **Concrete Waste card** - a section between Position and Break-Even (`_concrete_waste()` in dashboard.py), reading the two CompanyHealth waste workbooks (`Concrete_Waste_2026.xlsx` RP, `Concrete_Waste_MFD_2026.xlsx` MFD) at request time. RAW value columns only (the derived cells are formulas, so totals are recomputed here): RP over-plan % + concrete $ over + hottest super, MFD delivered-vs-ordered, and a $-over-by-super bar. Skips cleanly when the workbooks are absent (developer clone). Workbooks are built outside the repo (session scripts off the live General List + PO tracker + QBO).
