...
| `invoice_project_audit.py` | Read-only QBO. Invoices billed to the WRONG customer for the job their memo names (the RP6586 lot-prep invoice sat on the parent builder, 2026-09-08): who it is billed to (the project's parent / another project / unrelated), how many invoices the project already has under it (none = clearly missing), memos naming a job with no QBO customer. → `<CompanyHealth>/Invoices Off Project.xlsx`. Fixing is a manual QBO edit; `/invoice-audit` runs and reports it. |
| `rp_stage_scan.py` | Read-only QBO. Every RP job's invoicing kind by the one rule in `shared/rp_invoicing` - one-invoice (extras allowed) vs scope-based (the slab billed per stage) - with count, stage scopes, span and the RP WIP TYPE. → `<CompanyHealth>/RP Invoicing Stages.xlsx`. |
| `bill_payment_drift_audit.py` | Read-only over the QBO mirror. **Checks QBO rewrote after they were paid** - edit or delete a paid bill and QBO silently unapplies the check, the re-entered copy reads as owed again, the loan / vendor credit on the check is freed, and QBO later auto-applies the floating money to the next bill (worked example: check 48314, 2026-09-24). Flags FLOATING · LATE LINK (applied to a bill dated after the check) · COPY OPEN · CREDIT, with the cause when the mirror holds it; subs first. `--check <#>` prints one check's story, `--subs`. Refresh the mirror first. → `<CompanyHealth>/Bill Payment Drift.xlsx`. The copy match is tight for subs (one bill a week), loose for high-volume suppliers. |
| `job_vendor_report.py` | Read-only QBO. Every job cost in a date window grouped by vendor - the answer to a PM's "Transaction List by Vendor" with nothing missing. `--compare <their file>` adds a vendor-by-vendor diff sheet naming what each side is missing and why. Line amounts only, never bill totals. |
| `pnl_line_level_audit.py` | Read-only QBO. Proves every completed project P&L used LINE amounts, not bill totals - the failure that turns a profitable job into a fake disaster. Re-derives each job's cost from QBO independently, prints what whole-bill attribution WOULD have given, and exits non-zero if a delivered workbook disagrees. Run after any attribution change or P&L batch. |
